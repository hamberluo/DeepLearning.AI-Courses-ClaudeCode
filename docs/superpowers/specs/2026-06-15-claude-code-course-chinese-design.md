...
## 6. 阶段③ 详细设计（中文女声朗读 MP3）

### 6.1 决策：先做 MP3 朗读，manim 降级为可选

原计划做 manim 动画，但权衡内容类型后改为**给全站每页加中文女声朗读 MP3**（参考 GoGBA-doc 同款）：

- **内容类型决定形态**：本课讲「怎么操作终端 AI 工具」，其「过程演示」已由站内 `ClaudeReplay` 等交互组件还原，比 manim 重画更贴近真实、还能动手。manim 擅长的是抽象/数学/空间概念（如 GBA 内核），在这里性价比低、不对味。
- **MP3 几乎白捡**：复用现成脚本与声线，把站点补成「读 + 听 + 动手」的完整产品；正好满足最初「中文女声 1.5×」诉求。
- **manim 仅在要发独立视频（B 站 / YouTube）时才考虑**，且那种场景「录屏 + 配音」往往优于 manim。

### 6.2 实现

- 引擎：**edge-tts**（微软 Edge 免费 TTS），声线 `zh-CN-XiaoxiaoNeural`，语速 `+50%`（≈1.5 倍速）。
- 脚本：`website/tools/build-listen-audio.py` 读 `guide/*.md` → 清洗成朗读稿（跳过 info 块、把交互组件转成提示语、去代码块/表格、修正术语读法）→ 生成 `website/public/audio/NN-slug.mp3`。
- 播放器：`AudioPlayer.vue`（每页顶部嵌入，带浮动迷你播放器、可调倍速并记忆、进度条）。
- **MP3 不进 git**（根 `.gitignore` 忽略 `*.mp3`）：本地生成供预览，部署时由 GitHub Actions 在构建前用 edge-tts 自动生成。

## 7. 暂不做（YAGNI）

- 不做视频下载（只要字幕；正片在 YouTube 可看）。
- 不用 YouTube 机翻中文字幕。
- 暂不做 manim 动画（见 §6.1，降级为发视频平台时的可选项）。
